...
<div>
    <table class="table2">
        ...
        <c:forEach items="${result3.rows}" var="row3">
            <c:set value="${search2}" var="search2"></c:set>
            <tr>
                <td><c:out value="${row3.subject}"></c:out></td>
                <td><a href="Controller?page=view-article&title=${row3.title}&id=${row3.aid}"> <c:out value="${row3.title}"></c:out> </a></td>
                <td><c:set var = "string" value = "${fn:length(row3.author)}"/><c:out value="${fn:substring(row3.author, 0, 3)}***${fn:substring(row3.author, string-8, string)}"></c:out></td>
                <td><fmt:formatDate value="${row3.time}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                <sql:query var="result6" dataSource="${ db}">
                    select * from visit where aid="${ row3.aid}";
                </sql:query>

                <c:set var="q" value="0"></c:set>
                <c:forEach items="${ result6.rows}" var="row3">
