...
<sql:query var="result" dataSource="${ db}">
    select art.aid,art.title,art.authorName,art.time,SUM(lov.prefer) AS popular FROM love_article lov,article art where lov.aid=art.aid and art.hide!="Yes"  group BY title order by popular DESC;
</sql:query>
...
<td><c:set var = "string" value = "${ fn:length(row.authorName)}"/><c:out value="${ fn:substring(row.authorName, 0, 3)}***${ fn:substring(row.authorName, string-8, string)}"></c:out></td>
<td><c:out value="${ row.time}"></c:out></td>
<td><c:out value="${ row.popular}"></c:out></td>
</tr>
</c:forEach>
</table>
</div>
